...
class _Translations$start_day$ru implements Translations$start_day$en {
	...
	@override String get last_session_stats_title => 'ИТОГИ ВЧЕРАШНЕГО ДНЯ';
	...
	@override TextSpan freeze_applied_last_advice({required InlineSpanBuilder accent}) => TextSpan(children: [
		const TextSpan(text: 'Сыграйте сегодня — '),
		accent('без заморозок серия может прерваться уже завтра'),
	]);
...
extension on TranslationsRu {
	dynamic _flatMapFunction(String path) {
		return switch (path) {
			...
			'start_day.last_session_stats_title' => 'ИТОГИ ВЧЕРАШНЕГО ДНЯ',
