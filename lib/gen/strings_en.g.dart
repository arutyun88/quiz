...
class Translations$start_day$en {
	...
	/// ru: 'За вчера нет результатов'
	String get no_previous_summary_notice => 'No results for yesterday';

	/// ru: 'Завершите сегодняшний выпуск — ${accent(завтра здесь появятся результаты)}'
	TextSpan no_previous_summary_advice({required InlineSpanBuilder accent}) => TextSpan(children: [
		const TextSpan(text: 'Complete today\'s edition — '),
		accent('your results will appear here tomorrow'),
	]);
	...
	/// ru: 'Сыграйте сегодня — ${accent(без заморозок серия может прерваться уже завтра)}'
	TextSpan freeze_applied_last_advice({required InlineSpanBuilder accent}) => TextSpan(children: [
		const TextSpan(text: 'Play today — '),
		accent('with no freezes left, your streak could end tomorrow'),
	]);
